...
async function applyUpdatedCoordinates() {
  try {
      ...
      for (const valueSet of valuesSets) {
        try {
          ...
          if (parts.length >= 12) {
            ...
            // Create coordinates string for the existing table structure
    ...
    if (fetchError) {
      ...
      if (allRoutes.length > 0) {
        ...
        allRoutes.slice(0, 5).forEach(route => {
          ...
          console.log(`     Coordinates: ${route.coordinates ? JSON.parse(route.coordinates).length : 0} points`);
        });
      }
    }

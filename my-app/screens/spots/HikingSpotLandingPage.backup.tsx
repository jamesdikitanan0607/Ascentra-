...
export default function HikingSpotLandingPage({ navigation, route }: HikingSpotLandingPageProps) {
  ...
  // Function to handle route selection and map updates
  const updateSelectedRoute = useCallback((route: any) => {
    if (!route) return;
    ...
    // Update map coordinates if start coordinates are available
    if (route.start_coordinates) {
      setCoordinates({
        latitude: route.start_coordinates.latitude,
        longitude: route.start_coordinates.longitude
      });
    }
    
    // Scroll to trail info section
    if (trailInfoYRef.current && scrollViewRef.current) {
      scrollViewRef.current.scrollTo({ y: trailInfoYRef.current - 20, animated: true });
    }
  }, [hiking_spot_id, setSelectedTrail]);
              ...
              {trailRoutes && trailRoutes.length > 0 ? (
                <AvailableRoutes
                  ...
                  selectedRoute={selectedTrail ? {
                    ...
                    route_name: selectedTrail.name || 'Unnamed Route',
                    difficulty: (selectedTrail.difficulty || 'Moderate') as 'Easy' | 'Easy-Moderate' | 'Moderate' | 'Hard' | 'Very Hard',
                    distance: selectedTrail.length || 0,
                    elevation_gain: selectedTrail.elevation_gain || 0,
                    estimated_duration: selectedTrail.estimated_time || 0,
                    route_description: selectedTrail.description || '',
                    highlights: selectedTrail.description || '',
                    route_color: '#FF6B6B',
                    start_coordinates: selectedTrail.start_coordinates || { latitude: 0, longitude: 0 },
                    end_coordinates: selectedTrail.end_coordinates || { latitude: 0, longitude: 0 },
                    waypoints: selectedTrail.waypoints || '',
                    coordinates: selectedTrail.route_coordinates?.map(coord => [
                      coord.longitude || 0,
                      coord.latitude || 0
                    ]) || []
                  } : null}
                  onRouteSelect={(route) => {
                    const normalizedTrail = {
                      ...
                      route_coordinates: route.coordinates?.map(coord => ({
                        latitude: coord[1] || 0,
                        longitude: coord[0] || 0
                      })) || []
                    };

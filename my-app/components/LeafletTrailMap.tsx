...
<body>
    ...
    <script>
        ...
        // Create custom icons
        const createCustomIcon = (color = '#FF0000', size = 'medium') => {
            const iconSize = size === 'large' ? [25, 41] : [20, 32];
            const iconAnchor = size === 'large' ? [12, 41] : [10, 32];
        ...
        trailRoutes.forEach(route => {
            // Create polyline for trail path
            ...
            const popupContent = \`
                <div class="trail-popup">
                    ...
                    <div class="difficulty \${route.difficulty.replace(/\\s+/g, '-')}">\${route.difficulty}</div>
        ...
        window.addEventListener('message', (event) => {
            try {
                ...
                switch (data.type) {
                    case 'focusOnSpot':
                        focusOnSpot(data.spotId, data.coordinates);
                        break;
                    case 'selectTrail':

...
export default function TrailMapView({
  ...
  const prepareTrailsData = () => {
    ...
    return trailsToShow.map(trail => {
      ...
      // Extract full trail coordinates from geojson_path
      let trailCoordinates = [];
      if (trail.geojson_path && trail.geojson_path.coordinates && Array.isArray(trail.geojson_path.coordinates)) {
        trailCoordinates = trail.geojson_path.coordinates.map(coord => [coord[1], coord[0]]); // Convert [lng, lat] to [lat, lng]
      } else if (trail.route_coordinates && Array.isArray(trail.route_coordinates)) {
        trailCoordinates = trail.route_coordinates.map(coord => [coord.latitude, coord.longitude]);
      } else {
      ...
      return {
        ...
        startCoords,
        endCoords,
        trailCoordinates, // Add full trail path
        color: DIFFICULTY_COLORS[trail.difficulty as keyof typeof DIFFICULTY_COLORS] || '#007AFF',
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (data.type === 'mapReady') {
        setMapReady(true);
      } else if (data.type === 'trailSelected' && onTrailSelect && typeof onTrailSelect === 'function') {
    ...
    <body>
      ...
      <script>
        ...
        function addTrail(trail) {
          ...
          const { id, name, difficulty, distance_km, duration_hr, highlights, startCoords, endCoords, trailCoordinates, color, isSelected } = trail;

          // Create polyline using full trail coordinates
          const coordinates = trailCoordinates && trailCoordinates.length > 1 ? trailCoordinates : [startCoords, endCoords];
          ...
          const startMarker = L.marker(startCoords, {
            icon: createMarkerIcon('#4CAF50', 'S', isSelected)
          }).addTo(map);

          startMarker.bindPopup(\`
            <div class="popup-content">
              <div class="popup-title">\${name}</div>
              <div class="popup-subtitle">🟢 Trail Start</div>
          ...
          // End marker (red with E label)
          const endMarker = L.marker(endCoords, {
            icon: createMarkerIcon('#F44336', 'E', isSelected)
          }).addTo(map);

          endMarker.bindPopup(\`
            <div class="popup-content">
              <div class="popup-title">\${name}</div>
              <div class="popup-subtitle">🔴 Trail End</div>
